...
function fitnmf(G, L_all, L, H, k; α = 0, β = 0, λ = [0, 0], μ = [0, 0], iter = 500, print_iter = 50, initialize = :nndsvd, δ = 1e-5, dictionary = false, η = 1.0, U_init = nothing, V_init = nothing)
    ...
    @showprogress for it = 1:iter
        ...
        U_new .= relu.(U .* (G*V + λ[1]*A1*U + α*A*U*V'*V + β*H*V)) ./ (α*D*U*V'*V + U*V'*V + λ[1]*(D1*U) .+ μ[1] .+ δ)
        if dictionary
...
function fitntf(G, L, L_g, H, λ, μ, α, β, k; iter = 250, print_iter = 50, dictionary = false, δ = 1e-5, η = 1.0)
    ...
    S = tocu(S)
    # initialization 
    factor_cp = tl_decomp.non_negative_parafac(tocpu(G), rank = k, init = "svd", n_iter_max = 1, random_state = 0)
    for i = 1:length(A)
