...
TE = args["gpu"] ? estimate_TE_cu(X, 1:size(X, 2), 1:size(X, 2), Array(P_sp), Array(QT_sp), Array(R_sp); wclr=true, showprogress=true) :
                       estimate_TE(X, 1:size(X, 2), 1:size(X, 2), P_sp, QT_sp, R_sp; wclr=true, showprogress = true) 

@info "Denoising..."
_cu = args["gpu"] ? x -> CUDA.cu(Array(x)) : x -> x
G = Array(fitsp(_cu(TE), _cu(L); λ1 = args["lambda1"], λ2 = args["lambda2"], maxiter = args["maxiter"]))
