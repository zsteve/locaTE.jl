...
function cdf_norm(x::AbstractArray, A::AbstractMatrix; fits_reg_all = nothing, fits_targ_all = nothing)
    ...
    if fits_targ_all === nothing fits_targ_all = [cdf_fit(vcat(A[1:j-1, j], A[j+1:end, j])) for j = 1:size(A, 2)] end
...
function apply_cdf_norm(x::AbstractArray, A::AbstractMatrix)
    ...
    fits_targ_all = [cdf_fit(vcat(A[1:j-1, j], A[j+1:end, j])) for j = 1:size(A, 2)]
