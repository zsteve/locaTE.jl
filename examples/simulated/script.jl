...
# We load locaTE.jl and other required packages
using NPZ
using NNlib
...
# ## Construct kNN graph and Laplacian
kdtree = KDTree(X_pca')
...
w = normalize(vec(sqrt.(sum(TE_clr .^ 2; dims = 2))), 1) # weights (optional), we compute them here but do not use them in the next step
